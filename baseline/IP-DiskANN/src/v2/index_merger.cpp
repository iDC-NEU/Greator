...
#define pages_per_read (4096 * 2)   // index_file pages per read
...
namespace diskann {
  ...
  StreamingMerger<T, TagT>::StreamingMerger(
    ...
    this->id_map = id_map;
    // omp_init_lock(&prune_time_mutex);
    // omp_init_lock(&in_nodes_mutex);
    // omp_init_lock(&merge_nodes_mutex);
    // omp_init_lock(&free_ids_mutex);
    // omp_init_lock(&cout_mutex);
    // omp_init_lock(&compute_pq_dists_mutex);
  ...
  StreamingMerger<T, TagT>::~StreamingMerger() {
    ...
    // double e1 = (double) start.elapsed() / 1000000.0;
    delete this->disk_delta;
    // double e2 = (double) start.elapsed() / 1000000.0;
    for (auto &delta : this->mem_deltas) {
      delete delta;
    }
    // double e3 = (double) start.elapsed() / 1000000.0;
    aligned_free((void *) this->thread_pq_scratch);
    // double e4 = (double) start.elapsed() / 1000000.0;
    for (auto &data : this->mem_data) {
      // delete[] data;
      aligned_free((void *) data);
    }
    // double e5 = (double) start.elapsed() / 1000000.0;
    // std::cout << "e1: " << e1 << " e2: " << e2 - e1 << " e3: " << e3 - e2
    //           << " e4: " << e4 - e3 << " e5: " << e5 - e4 << std::endl;
    // omp_destroy_lock(&prune_time_mutex);
    // omp_destroy_lock(&in_nodes_mutex);
    // omp_destroy_lock(&merge_nodes_mutex);
    // omp_destroy_lock(&free_ids_mutex);
    // omp_destroy_lock(&cout_mutex);
    // omp_destroy_lock(&compute_pq_dists_mutex);
  }
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_INSERT_THREADS)
  ...
  template<typename T, typename TagT>
  void StreamingMerger<T, TagT>::offset_iterate_to_fixed_point_with_C(
      const T *vec, const uint32_t Lsize,
      std::vector<Neighbor>         &expanded_nodes_info,
      tsl::robin_map<uint32_t, T *> &coord_map, const uint32_t c) {
    std::vector<Neighbor> exp_node_info;
    exp_node_info.reserve(2 * Lsize);
    tsl::robin_map<uint32_t, T *> cmap;
    // first hit PQ iterate to fixed point
    ...
    uint32_t num = c < exp_node_info.size() ? c : exp_node_info.size();

    expanded_nodes_info.reserve(num);
    expanded_nodes_info.insert(expanded_nodes_info.end(), exp_node_info.begin(),
                               exp_node_info.begin() + num);
  ...
  void StreamingMerger<T, TagT>::process_deletes() {
    if (id_map == 2) {
      ...
      for (auto &dnode : disk_deleted_ids) {
        size_t sector_id = 1 + dnode / (this->disk_index->nnodes_per_sector);
        change_id_map[sector_id].push_back(dnode);
      }

      std::cout << "change_id_map size: " << change_id_map.size() << std::endl;
      // map->vec
      std::vector<uint32_t> page_id_vec;
      for (auto &pair : change_id_map) {
        page_id_vec.push_back(pair.first);
      }
      // std::sort(page_id_vec.begin(), page_id_vec.end());
      // calculate read-times
      size_t read_nums =
          (change_id_map.size() - 1 + pages_per_read) / pages_per_read;
      // pages_per_read
      ...
      for (size_t i = 0; i < read_nums; i++) {
        ...
        while (now_pages < pages_per_read &&
               (i * pages_per_read + now_pages) < change_id_map.size()) {
      ...
      std::fstream output_writer(this->temp_disk_index_path, std::ios::binary |
                                                                 std::ios::out |
                                                                 std::ios::in);
      // omp_lock_t   output_writer_mutex;
      // omp_init_lock(&output_writer_mutex);
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_N_THREADS)
        ...
        std::cout << "read_num: " << j << " / " << read_nums << " over!"
                  << std::endl;
      }
  ...
  void StreamingMerger<T, TagT>::populate_deleted_nhoods() {
    ...
    this->disk_index->scan_deleted_nodes(this->disk_deleted_ids, deleted_nodes,
                                         buf, this->delete_backing_buf, 1,
                                         this->id_map);

    std::cout << "Read scan over!" << std::endl;
    double e2e_time = ((double) scan_time.elapsed()) / (1000000.0);

    std::cout << "scan_deleted_nodes time: " << e2e_time << " s." << std::endl;
    ...
    std::cout << "start process_delete_node" << std::endl;
    uint32_t overR = 0;
    uint32_t notoverR = 0;
    tbb::concurrent_unordered_map<uint32_t,
                                  tbb::concurrent_unordered_set<uint32_t>>
                        new_edge;
    std::vector<char *> vbufs(MAX_N_THREADS);
    for (uint32_t i = 0; i < MAX_N_THREADS; i++) {
      alloc_aligned((void **) &vbufs[i], SECTOR_LEN, SECTOR_LEN);
    }
#pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_N_THREADS)
    for (size_t i = 0; i < deleted_nodes.size(); i++) {
      uint32_t               nowid = i;
      uint32_t               node_id = deleted_nodes[nowid].id;
      std::vector<uint32_t> &nbrs =
          this->disk_deleted_nhoods[deleted_nodes[nowid].id];  // live nbrs
                                                               // with deleted
                                                               // nodes
      DiskNode<T> &node = deleted_nodes[nowid];

      assert(node.id == node_id);
      T                            *coords = node.coords;
      uint32_t                      c = 128;
      std::vector<Neighbor>         nvisited;
      tsl::robin_map<uint32_t, T *> coord_map;

      offset_iterate_to_fixed_point_with_C(coords, this->l_index, nvisited,
                                           coord_map, c);

      std::vector<uint32_t> visited;
      std::vector<uint32_t> can;
      for (size_t j = 0; j < nvisited.size(); j++) {
        auto iter = this->disk_deleted_ids.find(nvisited[j].id);
        if (iter == this->disk_deleted_ids.end()) {
          visited.push_back(nvisited[j].id);
          if (can.size() < 50)
            can.push_back(nvisited[j].id);
        }
      }
      int   omp_thread_no = omp_get_thread_num();
      char *vbuf = vbufs[omp_thread_no];

      for (auto &innode : visited) {
        memset(vbuf, 0, SECTOR_LEN);
        uint32_t start_id =
            (innode / this->nnodes_per_sector) * this->nnodes_per_sector;
        std::vector<DiskNode<T>> nodes;
        this->disk_index->merge_read(nodes, start_id, 1, vbuf);
        uint32_t     insector_id = innode % this->nnodes_per_sector;
        DiskNode<T> &node = nodes[insector_id];
        bool         is_deletednode_visited = false;
        for (uint32_t nid = 0; nid < node.nnbrs; nid++) {
          if (node.nbrs[nid] == node_id) {
            is_deletednode_visited = true;
            break;
          }
        }
        if (!is_deletednode_visited)
          continue;
        std::vector<Neighbor> newnbrs;

        for (size_t j = 0; j < can.size(); j++) {
          if (can[j] == innode)
            continue;
          else {
            auto iter_right = coord_map.find(can[j]);
            auto iter_left = coord_map.find(innode);
            // HAS to be in coord_map since it was expanded during
            // iterate_to_fixed_point
            assert(iter_right != coord_map.end());
            assert(iter_left != coord_map.end());
            float djk = this->dist_cmp->compare(
                iter_left->second, iter_right->second, this->ndims);
            Neighbor nnode;
            nnode.id = can[j];
            nnode.distance = djk;
            newnbrs.push_back(nnode);
          }
        }

        std::sort(newnbrs.begin(), newnbrs.end());
        int already_in = 0;
        for (size_t j = 0; j < newnbrs.size(); j++) {
          int old_size = new_edge[innode].size();
          new_edge[innode].insert(newnbrs[j].id);
          int new_size = new_edge[innode].size();
      ...
      // process out_nbrs
      for (auto &nbr : nbrs) {
        std::vector<float> can_dists(can.size(), 0.0f);
        this->disk_index->compute_pq_dists(nbr, can.data(), can_dists.data(),
                                           (_u32) can.size(), pq_coord_scratch);
        std::vector<Neighbor> can_nbrs(can.size());
        assert(can.size() == can_dists.size());
        for (uint32_t j = 0; j < can.size(); j++) {
          can_nbrs[j].id = can[j];
          can_nbrs[j].distance = can_dists[j];
        }
        std::sort(can_nbrs.begin(), can_nbrs.end());
        if (can_nbrs.size() > 3)
          can_nbrs.resize(3);
        for (auto &ind : can_nbrs) {
          new_edge[ind.id].insert(nbr);
        }
      }
    }

    for (uint32_t i = 0; i < MAX_N_THREADS; i++) {
      aligned_free((void *) vbufs[i]);
    }
    for (auto &iter : new_edge) {
      for (auto &ind : iter.second) {
        this->new_edges[iter.first].push_back(ind);
      }
    }
    tbb::concurrent_unordered_map<uint32_t, tbb::concurrent_vector<uint32_t>>
                          change_id_map;
    std::vector<uint32_t> sector_ids;
    for (auto &iter : this->new_edges) {
      size_t sector_id = 1 + iter.first / (this->disk_index->nnodes_per_sector);
      change_id_map[sector_id].push_back(iter.first);
    }
    for (auto &iter : change_id_map) {
      sector_ids.push_back(iter.first);
    }
...
#pragma omp parallel for schedule(dynamic, 16) num_threads(MAX_N_THREADS)
    ...
    char *dbuf = nullptr;
    alloc_aligned((void **) &dbuf, (size_t) sector_ids.size() * SECTOR_LEN,
                  SECTOR_LEN);
    std::vector<std::vector<DiskNode<T>>> disk_nodess;
    this->disk_index->merge_read_through_diff_pages(
        disk_nodess, disk_tags, disk_deleted_ids, start_ids, dbuf);
...
#pragma omp parallel for schedule(dynamic, 16) num_threads(MAX_N_THREADS)
    for (size_t k = 0; k < sector_ids.size(); k++) {
      ...
      for (_u32 i = 0; i < vec.size(); i++) {
        ...
        auto &deltas = this->new_edges[disk_node.id];
        assert(deltas.size());
        uint32_t              nnbrs = disk_node.nnbrs;
        std::vector<uint32_t> nond_hood;
        for (size_t n = 0; n < nnbrs; n++) {
          auto iter = this->disk_deleted_nhoods.find(disk_node.nbrs[n]);
          if (iter == this->disk_deleted_nhoods.end()) {
            nond_hood.push_back(disk_node.nbrs[n]);
          }
        }
        std::unordered_set<uint32_t> nhood_set;
        nhood_set.insert(nond_hood.begin(), nond_hood.end());
        nhood_set.insert(deltas.begin(), deltas.end());
        std::vector<uint32_t> nhood(nhood_set.begin(), nhood_set.end());
        // nhood.insert(nhood.end(), nond_hood.begin(), nond_hood.end());
    ...
    this->disk_index->merge_write_through_diff_pages(start_ids, dbuf);
    aligned_free((void *) dbuf);
    this->new_edges.clear();

    std::cout << "process_nbrs time: "
              << ((double) scan_time.elapsed()) / (1000000.0)
              << " s. edges_num: " << new_edges.size() << "\n";
    std::cout << " OverR: " << overR << " NotoverR: " << notoverR << "\n";
    // free buf
    aligned_free((void *) buf);
  }
        ...
        *(disk_node.nbrs - 1) = disk_node.nnbrs;
        ...
        if (Modes_consolidate == 1) {
          ...
          for (uint32_t i = in_nodes_old_nbrs.size(); i < (_u32) final_size;
            ...
            for (uint32_t j = 0; j < this->disk_deleted_nhoods[did].size();
                 j++) {
          ...
          *(disk_node.nbrs - 1) = disk_node.nnbrs;
          ...
          // compute PQ dists and shrink
          std::vector<float> with_deleted_nodes_nbrs_dists(
              with_deleted_nodes_nbrs.size(), 0.0f);
          this->disk_index->compute_pq_dists(
              id, with_deleted_nodes_nbrs.data(),
              with_deleted_nodes_nbrs_dists.data(),
              (_u32) with_deleted_nodes_nbrs.size(), scratch);

          // 选择top-k个小指
          uint32_t k =
              std::ceil(((double) ((1.0 * deleted_nbrs_per_node) /
                                   id_nhood_old.size())) *
                        (this->range -
                         (id_nhood_old.size() - deleted_nbrs_per_node))) <
                      with_deleted_nodes_nbrs.size()
                  ? std::ceil(((double) ((1.0 * deleted_nbrs_per_node) /
                                         id_nhood_old.size())) *
                              (this->range -
                               (id_nhood_old.size() - deleted_nbrs_per_node)))
          ...
          std::vector<uint32_t> topk_index =
              findMinKIndexes(with_deleted_nodes_nbrs_dists, k);
          // uint32_t k =
          //     (this->range - (id_nhood_old.size() - deleted_nbrs_per_node)) <
          //             with_deleted_nodes_nbrs.size()
          //         ? (this->range -
          //            (id_nhood_old.size() - deleted_nbrs_per_node))
          //         : with_deleted_nodes_nbrs.size();
          ...
          // 最终邻居=原先邻居+k个新邻居
          uint32_t final_size = in_nodes_old_nbrs.size() + k;
          // copy back final nbrs
          disk_node.nnbrs = final_size;
          *(disk_node.nbrs - 1) = disk_node.nnbrs;
          for (uint32_t i = 0; i < (_u32) in_nodes_old_nbrs.size(); i++) {
            disk_node.nbrs[i] = in_nodes_old_nbrs[i];
          }
          for (uint32_t i = in_nodes_old_nbrs.size(); i < (_u32) final_size;
               i++) {
            disk_node.nbrs[i] =
                with_deleted_nodes_nbrs[topk_index[i -
                                                   in_nodes_old_nbrs.size()]];
          }

          // ##########22222222222222##########
        } else if (Modes_consolidate == 3) {
          // ##########33333333333333##########
          ...
          */
          ...
          // 从id_nhood中挑出与删除点邻居的边
          std::vector<uint32_t> with_deleted_nodes_nbrs;  // 入边点新插入的邻居
          std::vector<uint32_t> in_nodes_old_nbrs;        // 入边点原先的邻居
          ...
          *(disk_node.nbrs - 1) = disk_node.nnbrs;
          ...
          for (auto &nbr : id_nhood) {
            auto it = std::find(id_nhood_old.begin(), id_nhood_old.end(), nbr);
            if (it != id_nhood_old.end()) {
              in_nodes_old_nbrs.push_back(nbr);  // 原先邻居
            } else {
              with_deleted_nodes_nbrs.push_back(nbr);  // 删除点的邻居
            }
          }
          ...
          std::vector<_u32>  insert_ids;
          std::vector<float> insert_similaritys;

          for (auto &nbr : id_nhood_old) {
            auto iter = this->disk_deleted_nhoods.find(nbr);
            if (iter != this->disk_deleted_nhoods.end()) {
              std::vector<_u32> d_nbrs(iter->second.begin(),
                                       iter->second.end());  // 删除点的邻居
              for (auto &dnbr : d_nbrs) {
                auto iter = this->disk_deleted_ids.find(dnbr);
                if (iter != this->disk_deleted_ids.end()) {
                  continue;
                } else {
                  // 删除点的邻居是非删除点，候选插入
                  std::vector<_u32> dnn_nbrs =
                      topo[dnbr];  // 删除点的邻居的邻居们
                  float similarity = calculate_similarity(dnn_nbrs, d_nbrs);
                  auto  it =
                      std::find(insert_ids.begin(), insert_ids.end(), dnbr);
                  if (it != insert_ids.end()) {
                    int index = std::distance(insert_ids.begin(), it);
                    insert_similaritys[index] =
                        (insert_similaritys[index] + similarity) * 1.0 / 2.0;
                  } else {
                    insert_ids.push_back(dnbr);
                    insert_similaritys.push_back(similarity);
                  }
                }
              }
            }
          ...
          std::vector<std::pair<float, _u32>> combined;

          for (size_t i = 0; i < insert_ids.size(); ++i) {
            combined.push_back({insert_similaritys[i], insert_ids[i]});
          }
          std::sort(combined.rbegin(), combined.rend());
          ...
          *(disk_node.nbrs - 1) = disk_node.nnbrs;
          ...
          for (uint32_t i = in_nodes_old_nbrs.size(); i < (_u32) final_size;
               i++) {
            disk_node.nbrs[i] = combined[i - in_nodes_old_nbrs.size()].second;
          }
  ...
  void StreamingMerger<T, TagT>::compute_rename_map() {
    ...
    for (uint32_t mem_id = 0; mem_id < this->mem_data.size(); mem_id++) {
      ...
      for (uint32_t j = 0; j < this->mem_npts[mem_id]; j++) {
        ...
        rename_pairs[next_free_index].first =
            offset + j;  // 还是原始包含删除点的ID
        rename_pairs[next_free_index].second = new_id;  // 新ID
  ...
  std::vector<uint32_t> StreamingMerger<T, TagT>::get_edge_list(
      const uint32_t offset_id) {
    const uint32_t index_no =
        this->get_index_id(offset_id);  // 位于的sector——no
...
#pragma omp parallel for schedule(dynamic, 16) num_threads(MAX_N_THREADS)
    ...
    if (!id_map) {
      ...
      std::cout << "temp_index_in_path: " << this->temp_disk_index_path
                << "   ---> " << this->final_index_file;
      std::unique_ptr<char[]> sector_buf = std::make_unique<char[]>(SECTOR_LEN);
      std::cout << "\nfinal_index_file: " << final_index_file << std::endl;
      ...
      // std::map<int, std::vector<uint32_t>> graph_delta_mem;
      // std::map<int, std::vector<uint32_t>> graph_delta_disk;
      ...
      std::vector<bool> update_page;
      update_page.push_back(false);
...
//  针对每个disk——node
//  如果 idx
//  在rename_inverse中，那么就是新插入的点。且对应的coords和hood都是空的，需要插入邻居。
//         //  如果 idx
//         不在rename_inverse中，那么就是原本的点，需要更新邻居。
#pragma omp parallel for schedule(dynamic, 128) num_threads(MAX_N_THREADS)
        for (int64_t idx = 0; idx < (int64_t) disk_nodes.size(); idx++) {
          ...
          uint32_t              offset_id =
              this->rename_inverse(id);  // 新插入的点的id原本都超过max_npts
          // replaced by new vector, copy coords and proceed as normal
          if (offset_id !=
              std::numeric_limits<uint32_t>::max()) {  // 新插入点，need
                                                       // to insert
            // copy coords
            // offset_id 是插入时获得的id（超过disk_npts）
            // 首先获取offset_id在mem中的sector
            // id，然后计算在sector中的偏移量即local_id,最终返回
            const T *vec = this->get_mem_data(offset_id);  // 坐标
            assert(vec != nullptr);
            ...
            *(disk_node.nbrs - 1) = 0;  // also set on buffer
            ...
            if (!deltas.empty()) {
              update_page[page_num] = true;

              // graph_delta_mem[id] = deltas;
            }
          ...
          // this->rename(nhood); // skipping since get_edge_list() renames
          // delta edges

          // prune neighbor list ONLY if exceeding graph max out-degree
          // (`range`)
          if (nhood.size() > this->range) {
            std::vector<float>    dists(nhood.size(), 0.0f);
            std::vector<Neighbor> pool(nhood.size());
          ...
          *(disk_node.nbrs - 1) = (_u32) nhood.size();  // write to buf
          // std::cout << "prune_id: " << id << " nnbrs: " << nhood.size()
          //           << " nbrs: ";
          for (uint32_t i = 0; i < disk_node.nnbrs; i++) {
            disk_node.nbrs[i] = nhood[i];
            // std::cout << nhood[i] << " ";
          ...
          memcpy(disk_node.nbrs, nhood.data(),
                 disk_node.nnbrs * sizeof(uint32_t));
          // 	aux_writer << "disk_node.nbrs =  " << (uint64_t)
          // disk_node.nbrs
          // <<
          // ", id = ";
          uint32_t lcounts = 0;
        ...
        output_writer.write(
            buf, (uint64_t) SECTORS_PER_MERGE * (uint64_t) SECTOR_LEN);
      ...
      for (int i = 1; i < update_page.size(); i++)
        if (update_page[i]) {
          update_page_size++;
        }

      // std::cout << "Without improve_action's change: \n";
      // for (auto pair : graph_delta_mem) {
      //   int id = pair.first;
      //   std::cout << "mem_id: " << id << " nnbrs: " << pair.second.size()
      //             << " 's neighborhoods: ";
      ...
      //   std::cout << std::endl;
      // }
      // for (auto pair : graph_delta_disk) {
      //   int id = pair.first;
      //   std::cout << "disk_id: " << id << " nnbrs: " <<
      //   pair.second.size()
      //             << " 's neighborhoods: ";
      //   for (int i = 0; i < pair.second.size(); i++) {
      //     std::cout << pair.second[i] << " ";
      ...
      // std::cout << "changed_num_2: " << update_page_size
      //           << " mem_update_num: " << mem_update
      //           << " disk_update_num: " << disk_update << "\n";
      page_changed_num = update_page_size;
    } else if (id_map == 2) {
      // mem_update = 0;
      // disk_update = 0;
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_N_THREADS)
        for (size_t k = 0; k < sector_ids.size(); k++) {
          ...
          for (size_t i = 0; i < vec.size(); i++) {
            ...
            // replaced by new vector, copy coords and proceed as normal
            if (offset_id != std::numeric_limits<uint32_t>::max()) {
              // 新插入点，need to insert copy coords
              const T *vec = this->get_mem_data(offset_id);  // 坐标
              assert(vec != nullptr);
              memcpy(disk_node.coords, vec, this->ndims * sizeof(T));
  ...
  void StreamingMerger<T, TagT>::mergeImpl(uint32_t id_map) {
    uint32_t mem_use = 0;
    std::cout << "#" << mem_use++ << "Memory usage: " << std::endl;
    print_memory_usage();
    ...
    */
    ...
    this->new_edges.clear();
    // 处理mem_index中待插入的点中的删除点
    diskann::cout << "Computing rename-map.\n";

    // 存在mem上的新插入的点，其编号应该大于disk_max_num，而compute_rename_map()中会更新其编号。
    // 具体来说，从disk中空闲出来的编号（已删除）来更新新插入的点编号
    //  compute rename map
    // rename[offset_id]=ids, inverse[ids]=offset_id
    ...
    // call inserts
    // 插入操作涉及内存和磁盘
    // 首先针对每个插入点，从磁盘上找到其邻居节点，并剪枝。
    // 针对disk_delta，添加（hood,id)
    // 针对mem_delta，将邻居加入到graph[id]数组中。
    // get_io_info("get_begin");
    ...
    // 根据重命名映射表，更新delta_graph的点。只更新graph[i][j]，即只更新目的顶点。
  ...
  void StreamingMerger<T, TagT>::get_sector_with_deleted_nbrs_nodes(
    ...
    if (!infile.is_open()) {
      std::cerr << "无法打开输入文件: " << indir << std::endl;
    ...
    if (!infile) {
      std::cerr << "无法读取第 0 扇区的数据！" << std::endl;
      return;
    }
    // 提取 npts 和 R
    _u64 npts = 0;
...
#pragma omp parallel for schedule(dynamic, chunk_size) \
    num_threads(MAX_N_THREADS)
      for (size_t j = 0; j < pages_num; j++) {
        ...
        for (size_t k = 0; k < this->topological_nnodes_per_sector; k++) {
          ...
          if (this->disk_deleted_ids.find(node_id) !=
            ...
            // this->disk_deleted_nhoods.insert(
            //     std::make_pair(node_id, active_nbrs));
            continue;
          }
      ...
      // for (size_t d = 0; d < innode_ids.size(); d++) {
      //   _u32 page_id = 1 + innode_ids[d] / (this->nnodes_per_sector);
      //   sector_with_deleted_nbrs_nodes[page_id].push_back(innode_ids[d]);
      // }
    }
