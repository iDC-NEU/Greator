...
namespace diskann {
  ...
  MergeInsert<T, TagT>::MergeInsert(
      ...
      std::string working_folder, uint32_t merge_th)
  ...
  void MergeInsert<T, TagT>::switch_index(uint32_t id_map) {
    ...
    std::cout << "End_switch_index! use" << time << " s!" << std::endl;
    save();
    double time1 = (double) timer.elapsed() / 1000000.0;
    std::cout << "save use " << time1 - time << " s!" << std::endl;
    // start timer
    diskann::Timer timer1;
    std::cout << "#-1" << "Memory usage: " << std::endl;
    print_memory_usage1();
    construct_index_merger(id_map);
    std::cout << "#-2" << "Memory usage: " << std::endl;
    print_memory_usage1();
    double time2 = (double) timer1.elapsed() / 1000000.0;
    merge(id_map);
    std::cout << "#-3" << "Memory usage: " << std::endl;
    print_memory_usage1();
    double time3 = (double) timer1.elapsed() / 1000000.0 - time2;
    destruct_index_merger();
    std::cout << "#-4" << "Memory usage: " << std::endl;
    print_memory_usage1();
    // end timer
    double time4 = (double) timer1.elapsed() / 1000000.0 - time3;
    {
      std::shared_lock<std::shared_timed_mutex> lock(_index_lock);
    ...
    diskann::cout << "Merge time : " << timer.elapsed() / 1000000.0
                  << " construct use" << time2 << " merge use " << time3
                  << "destruct use " << time4 << " s" << std::endl;
  }
  ...
  void MergeInsert<T, TagT>::save_del_set() {
    ...
    {
      bool expected_value = false;
      _check_switch_delete.compare_exchange_strong(expected_value,
                                                   true);  // 切换删除集合
      std::unique_lock<std::shared_timed_mutex> lock(_delete_lock);
      if (_active_delete_set == 0) {
