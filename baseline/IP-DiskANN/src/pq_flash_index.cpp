...
#pragma omp critical
  ...
  void PQFlashIndex<T, TagT>::destroy_thread_data() {
    std::cout << "Destroying thread data-linsy" << std::endl;
...
#pragma omp parallel for schedule(dynamic, chunk_size) num_threads(10)
  ...
  void PQFlashIndex<T, TagT>::scan_deleted_nodes(
    ...
    for (auto dnode : delete_set) {
      // 扇区id+1
      uint32_t sector_id = 1 + dnode / (this->nnodes_per_sector);
    ...
    for (const auto &pair : dmap) {
      ...
      offset_t sector_offset = sector_id * SECTOR_LEN;  // 计算扇区起始偏移量

      // 初始化读取操作
      uint32_t                 n_scanned = 0;
      std::vector<AlignedRead> reads(1);
      reads[0].buf = buf;               // 指定缓冲区
      reads[0].len = SECTOR_LEN;        // 读取长度为一个扇区
      ...
      pmutex->ReadUnLock();
      for (auto &idx : dvec) {
        uint32_t offset_in_sector = idx % (this->nnodes_per_sector);

        // 指向节点数据的缓冲区位置
        char *node_buf = OFFSET_TO_NODE(buf, idx);
        char *buf_start =
    ...
    // }
    // for (auto &ind : dmap) {
    //   dvec.push_back(ind.first);
    // }
    // assert(dmap.size() == dvec.size());

    // int read_nums = (dmap.size() + sectors_per_scan - 1) / sectors_per_scan;
    // std::cout << "read_nums: " << read_nums << std::endl;
    // std::vector<std::vector<uint32_t>> sector_per_read(read_nums);
    // for (size_t i = 0; i < read_nums; i++) {
    //   int now_pages = 0;
    //   while (now_pages < sectors_per_scan &&
    //          (i * sectors_per_scan + now_pages) < dmap.size()) {
    //     sector_per_read[i].push_back(dvec[i * sectors_per_scan + now_pages]);
    //     // uint32_t sector_id = dvec[i * sectors_per_scan + now_pages];
    //     now_pages++;
    //   }
    // }
    // for (size_t i = 0; i < read_nums; i++) {
    //   std::vector<AlignedRead> reads(sector_per_read[i].size());
    //   for (size_t j = 0; j < sector_per_read[i].size(); j++) {
    //     uint32_t sector_id = sector_per_read[i][j];
    //     uint64_t sector_off = sector_id * SECTOR_LEN;
    //     reads[j].buf = buf + j * SECTOR_LEN;
    //     reads[j].len = SECTOR_LEN;
    //     reads[j].offset = sector_off;
    //   }
    //   this->reader->read(reads, ctx);
    //   for (size_t j = 0; j < sector_per_read[i].size(); j++) {
    //     uint32_t               sector_id = sector_per_read[i][j];
    //     std::vector<uint32_t> &vec = dmap[sector_id];
    //     char                  *page_buf = buf + j * SECTOR_LEN;
    //     for (auto &idx : vec) {
    //       uint64_t offset_in_sector =
    //           (uint64_t) idx % (this->nnodes_per_sector);
    //       char *node_buf = OFFSET_TO_NODE(page_buf, offset_in_sector);
    //       char *buf_start =
    //           backing_buf + (backing_buf_idx * backing_buf_unit_size);
    //       backing_buf_idx++;
    //       memcpy(buf_start, node_buf, max_node_len);
