...
#pragma omp parallel for schedule(static, 65536)
  ...
  void Index<T, TagT>::link(Parameters &parameters) {
    ...
    if (_num_frozen_pts > 0)
      ...
      _ep = calculate_entry_point();  //计算最靠近质心的点
...
//对于添加边后造成超R的邻居，且这些点一定都在visited里
...
#pragma omp parallel for schedule(dynamic)
  ...
  int Index<T, TagT>::insert_point(const T *point, const Parameters &parameters,
    ...
    //    assert(_has_built);
    std::vector<Neighbor>    pool;  //存的是location，而非tag
    std::vector<Neighbor>    tmp;
    tsl::robin_set<unsigned> visited;

    /*
      重复插入，先删除后插入。下面{}这段to delete repeated points
    */
    {
      std::shared_lock<std::shared_timed_mutex> lock(_update_lock);
  ...
  int Index<T, TagT>::lazy_delete(const TagT &tag) {
...

    {
      ...
      _delete_set.insert(_tag_to_location[tag]);  //存的是location
    }
