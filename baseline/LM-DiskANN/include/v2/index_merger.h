...
namespace diskann {
  ...
  class StreamingMerger {
   public:
    ...
    tsl::robin_set<uint32_t>        dnode_out_nbrs;
    tsl::robin_map<uint32_t, _u8 *> dout_node_pq_coords;
    _u8                            *insert_new_pq_coords = nullptr;
    std::vector<uint32_t>           insert_id_loc;  // [0-1000]=new_id
   ...
   private:
    ...
    // timing stuff
    std::vector<float>  insert_times, delta_times, search_times;
    std::mutex          _print_lock;
    std::vector<double> prune_times;
    // std::vector<double> mprune_times;
    bool _single_file_index = false;

    std::string TMP_FOLDER;
    double      write_times_delete = 0.0;
    double      write_times_merge = 0.0;
    int         prune_nums = 0;
    int         overR = 0;
    int         notoverR = 0;
  };
