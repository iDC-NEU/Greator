...
#define OFFSET_TO_NODE_PQ_COORDS(node_buf, R)           \
  (_u8 *) ((char *) (node_buf) + data_dim * sizeof(T) + \
           (R + 1) * sizeof(uint32_t))
namespace {
...
namespace diskann {
  ...
  template<typename T>
  DiskNode<T>::DiskNode(uint32_t id, T *coords, uint32_t *nhood, _u8 *pq_coords)
      : id(id) {
    this->coords = coords;
    this->nnbrs = *nhood;
    this->nbrs = nhood + 1;
    this->pq_coords = pq_coords;
  }
...
#pragma omp critical
      {
        ...
        _u64 pq_coord_alloc_size = MAX_N_CMPS * 2 * 32 * n_chunks;
        diskann::alloc_aligned((void **) &scratch.pq_coord_scratch,
                               pq_coord_alloc_size, 256);
  ...
  void PQFlashIndex<T, TagT>::destroy_thread_data() {
    for (auto &data : this->thread_data_backing_buf) {
      ...
      diskann::aligned_free((void *) scratch.pq_coord_scratch);
  ...
  void PQFlashIndex<T, TagT>::load_cache_list(
    ...
    _u64 pq_coord_cache_buf_len = num_cached_nodes * 32 * n_chunks;
    diskann::alloc_aligned((void **) &pq_coord_cache_buf,
                           pq_coord_cache_buf_len, 32);
    memset(pq_coord_cache_buf, 0, pq_coord_cache_buf_len);
    ...
    for (_u64 block = 0; block < num_blocks; block++) {
      ...
      for (auto &nhood : nhoods) {
        ...
        _u8 *node_pq_coords = OFFSET_TO_NODE_PQ_COORDS(node_buf, 32);
        _u8 *cached_pq_coords = pq_coord_cache_buf + node_idx * 32 * n_chunks;
        memcpy(cached_pq_coords, node_pq_coords, 32 * n_chunks);
        pq_coord_cache.insert(std::make_pair(nhood.first, cached_pq_coords));
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  void PQFlashIndex<T, TagT>::use_medoids_data_as_centroids() {
    ...
    if (pq_centroid_data != nullptr)
      aligned_free(pq_centroid_data);
    alloc_aligned(((void **) &pq_centroid_data), num_medoids * n_chunks, 1);
    std::memset(pq_centroid_data, 0, num_medoids * n_chunks);
    ...
    std::ifstream pq_compressed_vec_file(pq_compressed_name, std::ios::binary);
    for (uint64_t cur_m = 0; cur_m < num_medoids; cur_m++) {
      ...
      char *pq_medoid_buf = nullptr;
      alloc_aligned((void **) &pq_medoid_buf, n_chunks, n_chunks);
      size_t medoid_offset = 8 + medoid * n_chunks;
      pq_compressed_vec_file.seekg(medoid_offset);
      pq_compressed_vec_file.read(pq_medoid_buf, n_chunks);
      _u8 *medoid_pq_coords = new _u8[n_chunks];
      memcpy(medoid_pq_coords, pq_medoid_buf, n_chunks);
      for (_u32 i = 0; i < n_chunks; i++)
        pq_centroid_data[cur_m * n_chunks + i] = medoid_pq_coords[i];
      delete[] medoid_pq_coords;
      ...
      aligned_free(pq_medoid_buf);
    }
    pq_compressed_vec_file.close();
    // return ctx
    this->thread_data.push(data);
  ...
  int PQFlashIndex<T, TagT>::load(const char *index_prefix, _u32 num_threads,
    ...
    if (false == this->single_index_file) {
      ...
      pq_compressed_name = pq_compressed_vectors;
    ...
    // #ifdef EXEC_ENV_OLS
    //     diskann::load_bin<_u8>(files, pq_compressed_vectors, this->data,
    //     npts_u64,
    ...
    diskann::cout << "Load compressed vectors from file: "
                  ...
                  << " num points: " << num_points << " n_chunks: " << n_chunks
                  << std::endl;
...
// read index metadata
  ...
  size_t PQFlashIndex<T, TagT>::cached_beam_search(
    ...
    for (uint32_t i = 0;
      ...
      if (this->num_frozen_points == 1 &&
          expanded_nodes_info[i].id == this->frozen_location)
        continue;
      // std::cout << expanded_nodes_info[i].id << " ";
      if (distances != nullptr) {
        distances[res_count] = expanded_nodes_info[i].distance;
      }
      if (res_tags != nullptr && this->tags != nullptr) {
  ...
  void PQFlashIndex<T, TagT>::disk_iterate_to_fixed_point(
    ...
    _u8  *pq_data_buf = query_scratch->pq_coord_scratch;
    _u64 &pq_data_buf_idx = query_scratch->pq_coord_idx;
    _mm_prefetch((char *) pq_data_buf, _MM_HINT_T1);
    // sector scratch
    ...
    auto cmp_dists_pq_by_lm = [this, pq_coord_scratch, pq_dists](
    ...
    auto cmp_dists_pq_by_lm_2 = [this, pq_dists](
                                    const unsigned *ids, _u8 *pq_coords,
                                    const _u64 n_ids, float *dists_out) {
      ::pq_dist_lookup(pq_coords, n_ids, this->n_chunks, pq_dists, dists_out);
      // std::cout << "Dists: ";
      // for (uint32_t i = 0; i < n_ids; i++)
      //   std::cout << dists_out[i] << " ";
      // std::cout << std::endl;
    };
    ...
    memcpy(pq_coord_scratch, pq_centroid_data + best_cur * n_chunks, n_chunks);
    cmp_dists_pq_by_lm(&best_medoid, 1, dist_scratch);
    retset[0].id = best_medoid;
    retset[0].distance = dist_scratch[0];
    retset[0].flag = true;
    visited.insert(best_medoid);
    // std::cout << "Medoid dist: " << dist_scratch[0] << std::endl;
    ...
    while (k < cur_list_size && num_ios < IO_LIMIT) {
      ...
      for (auto &cached_nhood : cached_nhoods) {
        ...
        auto global_cache_pq_iter = pq_coord_cache.find(cached_nhood.first);
        _u8 *node_fp_pq_coords = global_cache_pq_iter->second;
        _u8 *node_fp_pq_coords_copy =
            pq_data_buf + (pq_data_buf_idx * n_chunks);
        T *node_fp_coords = global_cache_iter->second;
        T *node_fp_coords_copy = data_buf + (data_buf_idx * aligned_dim);
        data_buf_idx++;
        pq_data_buf_idx += 32;
        memcpy(node_fp_pq_coords_copy, node_fp_pq_coords, n_chunks);
        ...
        cpu_timer.reset();
        cmp_dists_pq_by_lm_2(node_nbrs, node_fp_pq_coords_copy, nnbrs,
                             dist_scratch);
      ...
      for (auto &frontier_nhood : frontier_nhoods) {
        ...
        _u8 *node_fp_pq_coords = OFFSET_TO_NODE_PQ_COORDS(node_disk_buf, 32);
        _u8 *node_fp_pq_coords_copy =
            pq_data_buf + (pq_data_buf_idx * n_chunks);
        assert(pq_data_buf_idx < MAX_N_CMPS * 2 * 32);
        pq_data_buf_idx += 32;
        memcpy(node_fp_pq_coords_copy, node_fp_pq_coords, 32 * n_chunks);
  ...
  template<typename T, typename TagT>
  void PQFlashIndex<T, TagT>::compute_pq_dists_by_pq_coords(
      const _u8 *src_pq_coords, _u8 *des_pq_coords, float *fp_dists,
      const _u32 count, uint8_t *aligned_scratch) {
    __sync_fetch_and_add(&this->cmp_count, count);

    // compute distances
    this->pq_table.compute_distances(src_pq_coords, des_pq_coords, fp_dists,
                                     count);
  }
  template<typename T, typename TagT>
    ...
    for (_u32 i = 0; i < n_sectors; i++) {
      ...
      for (_u32 j = 0; j < nnodes_per_sector && cur_node_id < this->num_points;
        ...
        disk_nodes.emplace_back(cur_node_id, OFFSET_TO_NODE_COORDS(node_buf),
                                ...
                                OFFSET_TO_NODE_PQ_COORDS(node_buf, 32));
        cur_node_id++;
      }
    }
  ...
  void PQFlashIndex<T, TagT>::scan_deleted_nodes(
    ...
    uint64_t backing_buf_unit_size = this->max_node_len;
        ...
        for (uint32_t j = 0;
          ...
          if (delete_set.find(n_scanned) != delete_set.end()) {
            ...
            DiskNode<T> node(n_scanned, OFFSET_TO_NODE_COORDS(buf_start),
                             ...
                             OFFSET_TO_NODE_PQ_COORDS(buf_start, 32));
            // std::cout << "Delete_node #" << didx++ << " : " << n_scanned
            //           << std::endl;
  ...
  void PQFlashIndex<T, TagT>::reload_index(
    ...
    // max_degree = ((max_node_len - data_dim * sizeof(T)) / sizeof(unsigned)) -
    // 1;
    max_degree = 32;
