...
#define PER_THREAD_BUF_SIZE (uint64_t) (SECTORS_PER_MERGE * 64 * 4)
...
namespace diskann {
  ...
  StreamingMerger<T, TagT>::~StreamingMerger() {
    ...
    aligned_free((void *) this->insert_new_pq_coords);
  }
  ...
  void StreamingMerger<T, TagT>::process_inserts_pq() {
    ...
    uint32_t new_insert_num = this->free_ids.size();
    insert_id_loc.resize(new_insert_num);
    uint32_t insert_new_pq_node_count = 0;
    alloc_aligned((void **) &insert_new_pq_coords,
                  (uint64_t) pq_nchunks * new_insert_num, 1);
    ...
    for (uint32_t i = 0; i < this->mem_data.size(); i++) {
      ...
      assert(count == new_insert_num || count == 0);
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_N_THREADS)
      ...
      for (int32_t j = 0; j < (int32_t) count; j++) {
        ...
        insert_id_loc[j] = j_renamed;
        memcpy(this->insert_new_pq_coords + (uint64_t) j * pq_nchunks,
               j_pq_coords.data(), pq_nchunks);
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_INSERT_THREADS)
    ...
    while (cur_alpha <= alpha && result.size() < range) {
      ...
      while (result.size() < range && (start) < pool.size() && start < maxc) {
        ...
        for (uint32_t t = start + 1; t < pool.size() && t < maxc; t++) {
          ...
          //                                    scratch);
          this->disk_index->compute_pq_dists_by_pq_coords(
              p.pq_coords, pool[t].pq_coords, &djk, 1);
          occlude_factor[t] =
              (std::max) (occlude_factor[t], pool[t].distance / djk);
        }
        start++;
  ...
  void StreamingMerger<T, TagT>::process_deletes() {
    ...
    while (start_id < this->disk_npts) {
      ...
      io_time += ((double) io_timer.elapsed()) / (1000000.0);
      std::cout << "Process #" << start_id << std::endl;
#pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_N_THREADS)
      ...
      output_writer.flush();
      double dwt = ((double) dt.elapsed() / 1000000);
      write_times_delete += dwt;
    ...
    uint64_t max_node_len =
        (this->ndims * sizeof(T)) + sizeof(uint32_t) +
        (this->range * sizeof(uint32_t) + (this->range * pq_nchunks));
    ...
    double time_header = ((double) io_header.elapsed()) / (1000000.0);
    // free buf
    ...
    std::cout << "Delete_stage's write use " << write_times_delete << " s"
              << std::endl;
    io_time += write_times_delete;
    io_time += time_header;
    double mpt = std::accumulate(this->prune_times.begin(),
                                 this->prune_times.end(), 0.0);
    std::cout << "Delete_stage's io/compute: " << io_time << " " << e2e_time
              << std::endl;
    std::cout << "Delete_prune nums: " << this->prune_nums << std::endl;
    write_times_delete = 0;
    this->prune_nums = 0;
  }
  ...
  void StreamingMerger<T, TagT>::populate_deleted_nhoods() {
    ...
    for (auto &nhood : deleted_nodes) {
      ...
      _u8                  *pq_coords = nhood.pq_coords;
      _u64                  nchunks = this->disk_index->get_n_chunks();
      for (uint32_t i = 0; i < nhood.nnbrs; i++) {
        uint32_t id = nhood.nbrs[i];
        _u8     *node_pq_coords = pq_coords + i * nchunks;
        auto     iter = this->disk_deleted_ids.find(id);
        if (iter == this->disk_deleted_ids.end()) {
          non_deleted_nbrs.push_back(id);
          dnode_out_nbrs.insert(id);
          dout_node_pq_coords[id] = node_pq_coords;
        }
      }
  ...
  void StreamingMerger<T, TagT>::consolidate_deletes(DiskNode<T> &disk_node,
    ...
    if (this->is_deleted(disk_node)) {
      disk_node.nnbrs = 0;
      *(disk_node.nbrs - 1) = 0;
      // std::cout << "deleted_node_id: " << disk_node.id << std::endl;
      return;
    }
    if (dnode_out_nbrs.find(disk_node.id) == dnode_out_nbrs.end())
      return;
    ...
    bool     change = false;
    uint32_t new_add = 0;
    // std::vector<uint32_t> old_nbrs;
    // tsl::robin_map<uint32_t, _u8 *> node_pq_coords;
    for (uint32_t i = 0; i < id_nhood.size(); i++) {
      uint32_t nbr = id_nhood[i];
      ...
      if (iter != this->disk_deleted_nhoods.end()) {
        ...
        new_add += iter->second.size();
      } else {
        if (nbr != id)
          new_edges.insert(nbr);
        // old_nbrs.push_back(i);
      }
    }
    ...
    _u8 *des_pq_coords = nullptr;
    _u64 nchunks = this->disk_index->get_n_chunks();
    alloc_aligned((void **) &des_pq_coords, new_edges.size() * nchunks,
                  nchunks);
    uint32_t                           des_node_count = 0;
    tsl::robin_map<uint32_t, uint32_t> nbr_id_loc;
    for (uint32_t i = 0; i < disk_node.nnbrs; i++) {
      nbr_id_loc[id_nhood[i]] = i;
    }
    ...
    for (auto &nbr : new_edges) {
      ...
      if (iter != this->disk_deleted_ids.end()) {
        ...
        _u8 *pq_ptr = nullptr;
        auto it = dout_node_pq_coords.find(nbr);
        if (it != dout_node_pq_coords.end()) {
          // out_nbrs
          pq_ptr = it->second;
        } else {
          uint32_t loc = nbr_id_loc[nbr];
          assert(loc < disk_node.nnbrs);
          pq_ptr = disk_node.pq_coords + loc * nchunks;
        }
        memcpy(des_pq_coords + des_node_count * nchunks, pq_ptr, nchunks);
        des_node_count++;
      }
    }

    assert(des_node_count == id_nhood.size());
    assert(id_nhood.size() == new_edges.size());
    ...
    _u8 *src_pq_coords = nullptr;
    alloc_aligned((void **) &src_pq_coords, nchunks, 1);
    memcpy(src_pq_coords, dout_node_pq_coords[id], nchunks);

    // this->disk_index->compute_pq_dists(id, id_nhood.data(),
    //                                    id_nhood_dists.data(),
    //                                    (_u32) id_nhood.size(), scratch);
    this->disk_index->compute_pq_dists_by_pq_coords(
        src_pq_coords, des_pq_coords, id_nhood_dists.data(),
        (_u32) id_nhood.size());
    ...
    for (uint32_t i = 0; i < id_nhood.size(); i++) {
      ...
      cand_nbrs[i].pq_coords = des_pq_coords + i * nchunks;
      // std::cout << id_nhood_dists[i] << " ";
    }
    // std::cout << std::endl;
    ...
    *(disk_node.nbrs - 1) = disk_node.nnbrs;
    // _print_lock.lock();
    // std::cout << "old_id: " << disk_node.id << " nnbrs: " << disk_node.nnbrs
    //           << " nbrs: ";
    for (uint32_t i = 0; i < (_u32) pruned_nbrs.size(); i++) {
      disk_node.nbrs[i] = pruned_nbrs[i].id;
      _u8 *pq_ptr = pruned_nbrs[i].pq_coords;

      _u8 *des_ptr = disk_node.pq_coords + i * nchunks;
      memcpy(des_ptr, pq_ptr, nchunks);
    }

    // _print_lock.lock();
    // std::cout << new_final_live << " " << new_add << " "
    //           << (1.0 * new_final_live) / new_add << std::endl;
    // _print_lock.unlock();
    double prunetime = ((double) prune_timer.elapsed()) / (1000000.0);
    // this->prune_times.push_back(prunetime);
    aligned_free((void *) des_pq_coords);
    aligned_free((void *) src_pq_coords);
  }
  ...
  void StreamingMerger<T, TagT>::compute_rename_map() {
    ...
    for (uint32_t mem_id = 0; mem_id < this->mem_data.size(); mem_id++) {
      ...
      for (uint32_t j = 0; j < this->mem_npts[mem_id]; j++) {
        ...
        next_free_index++;
        // std::cout << "offset_id: " << j << " loc: " << new_id << std::endl;
      }
    }
  ...
  void StreamingMerger<T, TagT>::process_merges() {
    ...
    double io_time = 0;
    this->prune_times.clear();
    int fd = open(this->temp_pq_coords_path.c_str(), O_RDONLY);
...
#pragma omp parallel for schedule(dynamic, 16) num_threads(MAX_N_THREADS)
      for (int64_t idx = 0; idx < (int64_t) disk_nodes.size(); idx++) {
        ...
        if (offset_id != std::numeric_limits<uint32_t>::max()) {
          ...
          *(disk_node.nbrs - 1) = 0;  // also set on buffer
          ...
          // std::cout << "insert_node_id: " << disk_node.id << std::endl;
        } else {
          // not replaced
        ...
        if (nhood.size() > this->range) {
          __sync_fetch_and_add(&this->overR, 1);
          _u8 *des_pq_coords = nullptr;
          assert(nhood.size() == nnbrs + deltas.size());
          alloc_aligned((void **) &des_pq_coords,
                        nhood.size() * (_u64) pq_nchunks, 1);
          memcpy(des_pq_coords, disk_node.pq_coords, nnbrs * (_u64) pq_nchunks);
          for (uint32_t i = 0; i < deltas.size(); i++) {
            auto it = std::find(insert_id_loc.begin(), insert_id_loc.end(),
                                deltas[i]);
            assert(it != insert_id_loc.end());
            size_t loc = std::distance(insert_id_loc.begin(), it);
            _u8   *pq_ptr = insert_new_pq_coords + loc * pq_nchunks;
            memcpy(des_pq_coords + (nnbrs + i) * pq_nchunks, pq_ptr,
                   pq_nchunks);
          }
          Timer                 mtimer;
          ...
          //                                    thread_scratch);
          // _u8 *src_pq_coords = nullptr;
          const T             *data_coords = disk_node.coords;
          std::vector<uint8_t> src_pq_coords =
              this->disk_index->deflate_vector(data_coords);

          this->disk_index->compute_pq_dists_by_pq_coords(
              src_pq_coords.data(), des_pq_coords, dists.data(), nhood.size());
          for (uint32_t k = 0; k < nhood.size(); k++) {
            pool[k].id = nhood[k];
            pool[k].distance = dists[k];
            pool[k].pq_coords = des_pq_coords + k * pq_nchunks;
          ...
          double mtime = ((double) mtimer.elapsed() / 1000000);
          aligned_free((void *) des_pq_coords);
          // prune_times.push_back(mtime);
        } else {
          __sync_fetch_and_add(&this->notoverR, 1);
        }
        ...
        *(disk_node.nbrs - 1) = (_u32) nhood.size();  // write to buf
        ...
        _u8 *pq_ptr = nullptr;
        alloc_aligned((void **) &pq_ptr, nhood.size() * (_u64) pq_nchunks, 1);
        for (uint64_t i = 0; i < nhood.size(); i++) {
          uint64_t f_id = nhood[i];
          pread(fd, pq_ptr + i * pq_nchunks, pq_nchunks,
                f_id * pq_nchunks + sizeof(uint32_t) * 2);
        }
        memcpy(disk_node.pq_coords, pq_ptr, nhood.size() * (_u64) pq_nchunks);
        aligned_free((void *) pq_ptr);
      ...
      double mwtime = ((double) mergetime.elapsed() / 1000000);
      write_times_merge += mwtime;
      start_id = new_start_id;
    }
    close(fd);
    ...
    diskann::cout << "Time to merge the inserts to disk: " << e2e_time << "s."
                  << std::endl;
    std::cout << "Merge_time's write_time use " << write_times_merge << " s"
              << std::endl;
    io_time += write_times_merge;
    io_time += time_header;
    double mpt = std::accumulate(this->prune_times.begin(),
                                 this->prune_times.end(), 0.0);
    std::cout << "Patch_stage's io/compute: " << io_time << " " << e2e_time
              << std::endl;
    std::cout << "Patch_overR nums: " << this->overR
              << " notoverR: " << this->notoverR << std::endl;
    this->overR = 0;
    this->notoverR = 0;
    write_times_merge = 0;
  }
  ...
  void StreamingMerger<T, TagT>::merge(
    ...
    std::cout << "cached_bfs_levels over" << std::endl;

    this->disk_index->load_cache_list(cache_node_list);
    std::cout << "load_cached_list over" << std::endl;
    ...
    std::cout << "Start load mem-indices" << std::endl;
    // load mem-indices
  ...
  void StreamingMerger<T, TagT>::mergeImpl() {
    ...
    double scan_time = ((double) scan.elapsed()) / 1000000.0;
    std::cout << "Process_scan_time: " << scan_time << std::endl;
    get_io_info("get_end");
    ...
    double dtime = ((double) dtimer.elapsed()) / 1000000.0;
    // END -- graph on disk has NO deleted references, maybe some holes
    std::cout << "Process_delete_time: " << dtime << std::endl;
    ...
    copy_file(this->disk_index_in_path + "_pq_compressed.bin",
              temp_pq_coords_path);
    ...
    this->disk_index->set_num_point(new_max_pts);
    ...
    double crm_time = ((double) crm.elapsed()) / 1000000.0;
    std::cout << "Process_insert_time: " << crm_time << std::endl;
    ...
    int fd = open(this->temp_pq_coords_path.c_str(), O_WRONLY);

#pragma omp parallel for schedule(dynamic, 1) num_threads(MAX_N_THREADS)
    for (uint32_t i = 0; i < insert_id_loc.size(); i++) {
      uint32_t loc = insert_id_loc[i];
      assert(loc < npts_u32);
      _u8 *pq_ptr = insert_new_pq_coords + (uint64_t) i * pq_nchunks;
      _u64 offset = sizeof(uint32_t) * 2 + (_u64) loc * pq_nchunks;
      pwrite(fd, (char *) pq_ptr, pq_nchunks, offset);
    }
    // 所有线程完成写入后再关闭
    if (close(fd) == -1) {
      perror("close");
    ...
    double mtime = ((double) mtimer.elapsed()) / 1000000.0;
    // double mptime =
    //     std::accumulate(mprune_times.begin(), mprune_times.end(), 0.0);
    std::cout << "Process_merge_time: " << mtime << std::endl;
    // END -- graph on disk has NO deleted references, has newly inserted points
    std::cout << "ALL: 1)delete_1: " << scan_time << " 2)delete_2: " << dtime
              << " 3)insert: " << crm_time << " 4)merge: " << mtime
              << std::endl;
    ...
    if (this->_single_file_index) {
      ...
      //                            (uint64_t) ndims_u32);
      copy_file(temp_pq_coords_path, final_pq_coords_file);
